...
class InlineManager:
    ...
    async def _create_bot(self) -> None:
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            m = await conv.send_message(f"◍ SLOPPY ◍")
            r = await conv.get_response()
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    if re.search(r"@SLOPPY_[0-9a-zA-Z]{6}_Bot", button.text, re.I):
                        ...
                        m = await conv.send_message("SLOPPYInlayn")
                        r = await conv.get_response()
            ...
            for button in array_sum(form.get("buttons", [])):
                if button.get("_callback_data", None) == query.data:
                    if (
                        ...
                        await query.answer("SLOPPY sizga tegishli emas!")
                        return
    ...
    async def form(
        ...
        try:
            ...
            msg = (
                "🌉 <b>«SLOPPY»</b> tarafidan yaratilgan maxsus bot oʻlib qolgan.\n🌄 <b>«Iltimos!»</b> SLOPPY qayta oʻrnating."
            )
