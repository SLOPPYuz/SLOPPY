...
class CloudBackend:
    ...
    async def _find_data_channel(self):
        async for dialog in self._client.iter_dialogs(None, ignore_migrated=True):
            if dialog.name == f"🥷 SLOPPY-{self._me.user_id}-𝙳𝚊𝚝𝚊" and dialog.is_channel:
            ...
            return (
                await self._client(
                    CreateChannelRequest(
                        f"🥷 SLOPPY-{self._me.user_id}-𝙳𝚊𝚝𝚊",
                        "🥷 SLOPPY - 𝙳𝚊𝚝𝚊𝙱𝚊𝚣𝚊...",
                        megagroup=True,
                    )
                )
    ...
    async def _find_asset_channel(self):
        async for dialog in self._client.iter_dialogs(None, ignore_migrated=True):
            if (
                dialog.name == f"🥷 SLOPPY-{self._me.user_id}-𝙱𝚊𝚣𝚊"
    ...
    async def _make_asset_channel(self):
        async with self._anti_double_asset_lock:
            ...
            return (
                await self._client(
                    CreateChannelRequest(
                        f"🥷 SLOPPY-{self._me.user_id}-𝙱𝚊𝚣𝚊",
                        "🥷 SLOPPY - 𝙳𝚊𝚝𝚊𝙱𝚊𝚣𝚊...",
                        megagroup=True,
                    )
                )
    ...
    async def do_upload(self, data):
        ...
        if newmsg:
            await self._client.send_message(self.db, "<b>🥷 Log uchun guruh.</b>\n Guruhni oʻchirmang!")
